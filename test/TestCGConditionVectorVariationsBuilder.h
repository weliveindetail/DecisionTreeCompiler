...
void checkVariations(int pathNum, std::vector<uint32_t> actualVariations,
...
TEST(CGConditionVectorVariationsBuilder, RegularTree2) {
  ...
  checkVariations(0, variationsBuilder.run(paths[0]),
                  {0b000, 0b100}); // 2 -> 5 -> 11
  checkVariations(1, variationsBuilder.run(paths[1]),
                  {0b010, 0b110}); // 2 -> 5 -> 12
  checkVariations(2, variationsBuilder.run(paths[2]),
                  {0b001, 0b011}); // 2 -> 6 -> 13
  checkVariations(3, variationsBuilder.run(paths[3]),
                  {0b101, 0b111}); // 2 -> 6 -> 14
...
TEST(CGConditionVectorVariationsBuilder, RegularTree3) {
  ...
  //
  checkVariations(
      0, variationsBuilder.run(paths[0]),
      {//___D_BA     ___D_BA     ___D_BA     ___D_BA     ___D_BA     ___D_BA
       0b0000000, 0b0000100, 0b0010000, 0b0100000, 0b1000000, 0b0010100,
       0b0100100, 0b1000100, 0b0110000, 0b1010000, 0b1100000, 0b0110100,
       0b1010100, 0b1100100, 0b1110000, 0b1110100});
