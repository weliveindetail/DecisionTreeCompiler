...
TEST(CGConditionVectorVariationsBuilder, RegularTree2) {
  ...
  // 2 -> 5 -> 11
  checkVariations(0, variationsBuilder.run(paths[0]), {0b000, 0b100});

  // 2 -> 5 -> 12
  checkVariations(1, variationsBuilder.run(paths[1]), {0b010, 0b110});

  // 2 -> 6 -> 13
  checkVariations(2, variationsBuilder.run(paths[2]), {0b001, 0b011});

  // 2 -> 6 -> 14
  checkVariations(3, variationsBuilder.run(paths[3]), {0b101, 0b111});
...
TEST(CGConditionVectorVariationsBuilder, RegularTree3) {
  ...
  checkVariations(0, variationsBuilder.run(paths[0]),
                  flattenToVector(expectedVariations));
